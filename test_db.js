...
const supabaseKey =
	'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImtvdWJqZG5oYXpqdHlra21hbGZ3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjU5MjA2NTYsImV4cCI6MjA0MTQ5NjY1Nn0.oWcXG_M4EC5_nw6gchqN6LHujrxIn-nzPvpPsdEGsDc';

const supabase = createClient(supabaseUrl, supabaseKey);

async function test() {
	console.log('--- Testing mCliente table ---');
	const { data: cData, error: cError } = await supabase.from('mCliente').select('*').limit(1);
	...
	const { data: vData, error: vError } = await supabase.from('mVenta').select('*').limit(1);
