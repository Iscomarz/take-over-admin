<script>
    ...
    //export let image;
    export let venue;
    export let id; // Agregar el id del evento
</script>

<div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
    <a href={`/events/event/${id}`}>
        <img class="rounded-t-lg" src="\images\covers\portada-prueba.jpg" alt="" />
    </a>
    <div class="p-5">
        <a href={`/events/event/${id}`}>
            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{nombreEvento}</h5>
        </a>
