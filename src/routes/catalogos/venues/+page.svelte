...
<div class="min-h-screen bg-[#111111] p-8 text-white">
	<div class="max-w-7xl mx-auto space-y-6">
		<!-- Header Segment -->
		<div
			class="flex flex-col md:flex-row justify-between items-start md:items-center p-6 bg-stone-900 border border-stone-800 rounded-3xl shadow-xl w-full"
		>
			<div>
				<h1 class="text-2xl font-bold tracking-wider text-white">Catálogo de Venues</h1>
				<p class="text-sm text-stone-400 mt-1">
					Gestiona los lugares donde se realizan los eventos.
				</p>
			</div>
								...
								<tr class="hover:bg-stone-800/30 transition-colors">
									...
									<td class="px-6 py-4 text-stone-300">
										...
										{#if venue.url_direccion}
											<a
												href={venue.url_direccion}
												target="_blank"
												class="block text-xs text-green-400 hover:underline mt-1">Ver mapa</a
											>
										{/if}
									</td>
									...
									<td class="px-6 py-4">
										<span
											class={`px-2 py-1 text-xs rounded-lg font-medium border ${venue.activo ? 'bg-green-900/30 text-green-400 border-green-800' : 'bg-red-900/30 text-red-400 border-red-800'}`}
										>
...
{#if showModal}
	...
	>
		<div
			class="bg-stone-900 border border-stone-700 rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col"
		>
				...
				<form id="venueForm" on:submit={saveVenue} class="space-y-5">
					<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
						<div class="md:col-span-2">
							...
							<input
								type="text"
								bind:value={nombre_venue}
								required
								class="w-full bg-stone-800 text-white border border-stone-700 rounded-xl p-3 focus:ring-2 focus:ring-green-500/50 focus:border-transparent"
			...
			<div class="p-6 border-t border-stone-800 flex justify-end gap-3 bg-stone-900">
				<button
					type="button"
					on:click={resetForm}
					class="px-5 py-2.5 rounded-xl text-stone-300 font-medium hover:bg-stone-800 transition"
					>Cancelar</button
				>
				<button
					type="submit"
					form="venueForm"
					class="px-5 py-2.5 bg-green-900/40 text-green-300 border border-green-700/50 font-medium rounded-xl hover:bg-green-900/60 transition"
					>{editandoId ? 'Actualizar Venue' : 'Guardar Venue'}</button
				>
			</div>
		</div>
	</div>
